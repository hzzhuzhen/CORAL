...
# 训练器（SLM训练核心代码，继承了transformer的train，重写了其中一些方法）
class MyTrainer(Trainer):
    def _inner_training_loop(self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None):
        ...
        # Check if continuing training from a checkpoint 是否进行继续训练
            ...
            if epoch > 0:
                ...
                for j in range(num_labels):
                    ...
                    for manual_idx in manual_list:
                        if manual_idx not in index_j_sel:
                            index_j_sel = torch.cat([index_j_sel, torch.tensor([manual_idx]).cuda()])
                ...
                if args.learning_setting=='transductive' and epoch == 1:    
                    ...
                    with open("./feedback/pred_label_mr.pkl","wb") as f:
                        ...
                        pred_label_all_new = pred_label_all.copy()
                        if(len(np.where(manual_label != -1)[0])>0): #对空的情况特殊处理
                            pred_label_all_new[np.where(manual_label != -1)[0]] = manual_label[np.where(manual_label != -1)]
                        pickle.dump(pred_label_all_new,f)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        model.train() # 其中会调用_inner_training_loop
        inputs = self._prepare_inputs(inputs)

        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, inputs)  # 调用计算损失
