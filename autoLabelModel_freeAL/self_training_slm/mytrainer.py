...
# current path: /home/roger/codePlace/pythonCode/DIML/autoLabelModel_freeAL/CORALV1_0
# grandparent_path :/home/roger/codePlace/pythonCode/DIML/autoLabelModel_freeAL
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if epoch > 0:
                ...
                for j in range(num_labels):  # 从标签集范围中分别一一取出
                    ...
                    manual_label = []
                    with open(sys.path[0]+"/data/manual_label.npy", "rb") as f:
                        manual_label = np.load(f)
                    # 选择集合的选中集中，把manual_label 不为-1（作废），全部加进来。
                    chosen_list_sel[np.where(manual_label != -1)[0]] = 1
                    # 手工标注增加影响：将手工标注的都放入有标签集合
                    chosen_list[np.where(manual_label != -1)[0]] = 1
                ...
                if args.learning_setting == 'transductive' and epoch == 1:
                    ...
                    with open(sys.path[0]+"/CORALV1_0/self_trainning_slm"+"/feedback/right_list_mr.pkl", "wb") as f:
                        pickle.dump(chosen_idx_sel.tolist(), f)
                    # 演示集保存 demonstration retrieval
                    with open(sys.path[0]+"/CORALV1_0/self_trainning_slm"+"/feedback/demo_index_mr.pkl", "wb") as f:
                        pickle.dump(chosen_top_indices.tolist(), f)
                    # 聚类伪标签保存pseudo-labels for all samples
                    with open(sys.path[0]+"/CORALV1_0/self_trainning_slm"+"/feedback/pred_label_mr.pkl", "wb") as f:
                        with open(sys.path[0]+"/CORALV1_0/self_trainning_slm"+"/data/manual_label.npy", "rb") as fm:
