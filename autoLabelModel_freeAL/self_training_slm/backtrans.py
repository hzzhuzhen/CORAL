...
parser.add_argument('--task', default='mr', help='emotion classifaction')
args = parser.parse_args()
pipe = pipeline("translation", model="self_training_slm/models/Helsinki-NLP/opus-mt-tc-big-en-fr",device=0)
pipe_bt = pipeline("translation", model="self_training_slm/models/Helsinki-NLP/opus-mt-tc-big-fr-en",device=0)
with open(os.path.join("self_training_slm/data_in",args.task,"train.csv"), 'r') as file:
...
with open(os.path.join("self_training_slm/data_in",args.task,"train_chat_bt_new.csv"), 'w', newline='') as file:
